...
/* ================= STATE ================= */
const DEFAULT_STATE = () => ({
  day:1, money:120, rep:1, tidy:0, lifetimeFinds:0,
...
function hpBars(you, her){
  const bar = (hp, max) => {
    let s = '';
    for(let i=0;i<max;i++) s += i < hp ? '\u2764\ufe0f' : '\ud83d\udda4';
    return s;
  };
  return '<div style="display:flex;justify-content:space-between;font-size:15px;margin-bottom:8px">' +
    '<span>\ud83e\udd4a YOU ' + bar(Math.ceil(you),3) + '</span><span>' + bar(Math.ceil(her),3) + ' KAREN \ud83d\udc5c</span></div>';
}
async function karenBoxing(){
  let you = 3, her = 3, round = 1;
  await modal({ icon:'\ud83e\udd4a', title:'Queensberry rules', body:
    '<p>She hands her phone to a bystander to film. You put your detector down gently. The dog walker rings a bell that neither of you can hear.</p>',
    options:[{label:'Gloves up', gold:true}]});
  while(you > 0 && her > 0){
    const cues = [
      { k:'hay',   c:'She winds up a HUGE haymaker\u2026' },
      { k:'jab',   c:'She flicks out sharp little jabs\u2026' },
      { k:'guard', c:'She covers up behind the handbag\u2026' }];
    const cue = cues[Math.random()*3|0];
    const pick = await modal({ icon:'\ud83e\udd4a', title:'Round '+round,
      body: hpBars(you,her) + '<div class="quote">'+cue.c+'</div>',
      options:[{key:'jab',label:'JAB'},{key:'duck',label:'DUCK'},{key:'block',label:'BLOCK'}]});
    let msg = '';
    if(cue.k==='hay'){
      if(pick==='duck'){ her -= 2; msg = 'You duck the haymaker and counter. She spins like a shopping trolley with a bad wheel.'; }
      else if(pick==='block'){ you -= 0.5; msg = 'You block it. Your forearm files a complaint.'; }
      else { you -= 1; her -= 1; msg = 'You trade blows. Somewhere, a kookaburra laughs.'; }
    } else if(cue.k==='jab'){
      if(pick==='block'){ her -= 1; msg = 'You catch the jab and counter neatly. Textbook.'; }
      else if(pick==='duck'){ you -= 1; msg = 'You duck straight into it. Rookie error.'; }
      else { you -= 0.5; her -= 0.5; msg = 'A messy exchange of slaps. The bystander zooms in.'; }
    } else {
      if(pick==='jab'){ her -= 1; msg = 'You jab straight through the handbag defence.'; }
      else if(pick==='duck'){ msg = 'You duck under\u2026 nothing. She looks confused. So does the dog.'; }
      else { msg = 'You both stand there blocking. The crowd (one labrador) is bored.'; }
    }
    player.stamina = Math.max(0, player.stamina - 6);
    round++;
    if(you > 0 && her > 0)
      await modal({ icon:'\ud83e\udd4a', title:'\u2026', body: hpBars(you,her)+'<p>'+msg+'</p>', options:[{label:'Next round', gold:true}]});
  }
  if(her <= 0){
    state.karensSurvived++;
    state.rep = Math.max(0, state.rep - 1);
    await modal({ icon:'\ud83c\udfc6', title:'Winner by handbag forfeit',
      body:'<p>She scoops up the handbag, declares the whole thing assault, and power-walks to the car park. The footage, mercifully, is all sky.</p><p class="dim">Word gets around, though. Reputation down \u2605 \u2014 and someone may have rung the ranger.</p>',
      options:[{label:'Back to the signal', gold:true}]});
    eventTimer = Math.min(eventTimer, 12); flagTime = Math.max(flagTime, 5);
  } else {
    await modal({ icon:'\ud83e\udd15', title:'Down for the count',
      body:'<p>The haymaker you never saw. You come to with grass in your mouth and a labrador licking your ear. The footage is already captioned.</p><p class="dim">Stamina is gone. Maybe head home.</p>',
...
async function karenDialogue(){
  ...
  const line = K.lines[Math.random()*K.lines.length|0];
  const site = SITES[state.location];
  ...
  const choice = await modal({ icon:'📱', title:K.title,
    ...
    options: K.options.map(o=>({ key:o.key, label:o.label + (o.hint? ' <span class="dim small">— '+o.hint+'</span>':'') }))
      .concat([{ key:'box', label:'Settle it in the ring 🥊 <span class="dim small">— Queensberry rules</span>' }]) });
...
function animate(){
  ...
  if(running && !paused){
    ...
    camera.position.set(player.x, gy+1.62+bob, player.z);
